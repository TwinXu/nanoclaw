...
describe('DingTalkChannel', () => {
  ...
  describe('text extraction', () => {
    ...
    it('extracts richText with picture and text nodes', () => {
      const channel = new DingTalkChannel(createTestOpts());
      const msg = createRobotMessage({
        ...
        content: {
          richText: [
            { type: 'picture', downloadCode: 'dc_pic_123', pictureDownloadCode: 'pdc_pic_123' },
            { text: '\n' },
            { text: '@Bot' },
            { text: '看看这张图' },
          ],
      ...
      expect((channel as any).extractContent(msg)).toBe(
        '[图片 image_key=dc_pic_123 message_id=rt_msg_1] @Bot 看看这张图',
      );
    });

    it('returns placeholder for richText without content', () => {
      const channel = new DingTalkChannel(createTestOpts());
    ...
    it('extracts picture with metadata format', () => {
      const channel = new DingTalkChannel(createTestOpts());
      const msg = createRobotMessage({
        msgId: 'pic_msg_1',
        msgtype: 'picture',
        text: undefined,
        picture: { picURL: 'https://example.com/image.jpg' },
      });
      expect((channel as any).extractContent(msg)).toBe(
        '[图片 image_key=https://example.com/image.jpg message_id=pic_msg_1]',
      );
    });

    it('returns fallback for picture message without picURL', () => {
      const channel = new DingTalkChannel(createTestOpts());
      const msg = createRobotMessage({ msgtype: 'picture', text: undefined, picture: undefined });
      expect((channel as any).extractContent(msg)).toBe('[picture message]');
    });

    it('extracts file with metadata format', () => {
      const channel = new DingTalkChannel(createTestOpts());
      const msg = createRobotMessage({
        msgId: 'file_msg_1',
        msgtype: 'file',
        text: undefined,
        file: { fileName: 'report.pdf', downloadCode: 'dc_abc123' },
      });
      expect((channel as any).extractContent(msg)).toBe(
        '[文件 file_key=dc_abc123 file_name=report.pdf message_id=file_msg_1]',
      );
    });
  ...
  describe('downloadMedia', () => {
    it('downloads image from direct picURL', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      const result = await channel.downloadMedia(
        'msg_123',
      ...
      expect(result).toBe('req_001.jpg');
      expect(mockFetch).toHaveBeenCalledWith('https://example.com/photo.jpg');
      expect(mockWriteFileSync).toHaveBeenCalledWith(
        '/tmp/dest/req_001.jpg',
        expect.any(Buffer),
      );
    });

    it('downloads file via downloadCode (API resolve)', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ accessToken: 'tok_dl', expireIn: 7200 }),
      });
      // 2. Resolve downloadCode -> downloadUrl
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ downloadUrl: 'https://cdn.dingtalk.com/temp-file.pdf' }),
      });
      ...
      expect(result).toBe('req_002.pdf');
      // Should have called the resolve API
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.dingtalk.com/v1.0/robot/messageFiles/download',
        expect.objectContaining({
          method: 'POST',
          body: expect.stringContaining('dc_code_xyz'),
        }),
      );
      // Should have fetched the resolved URL
      expect(mockFetch).toHaveBeenCalledWith('https://cdn.dingtalk.com/temp-file.pdf');
    });

    it('returns null when direct fetch fails', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
    ...
    it('returns null when downloadCode resolve fails', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
    ...
    it('detects image extension from content-type', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      expect(result).toBe('req_ext.png');
    });

    it('detects file extension from content-disposition', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ downloadUrl: 'https://cdn.dingtalk.com/file' }),
      });
      // Fetch file with content-disposition
      mockFetch.mockResolvedValueOnce({
        ok: true,
        headers: new Headers({
      ...
      expect(result).toBe('req_disp.xlsx');
    });
  });
  ...
  describe('uploadMedia', () => {
    it('uploads image and returns media_id', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ accessToken: 'tok_up', expireIn: 7200 }),
      });
      // Upload
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ media_id: '@media_img_123', errcode: 0 }),
      });

      const result = await (channel as any).uploadMedia('/tmp/test.png', 'image');

      expect(result).toBe('@media_img_123');
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('oapi.dingtalk.com/media/upload'),
    ...
    it('returns null when upload fails', async () => {
      ...
      mockReadFileSync.mockReturnValue(Buffer.from('fake-file'));
      ...
      const result = await (channel as any).uploadMedia('/tmp/test.pdf', 'file');

      expect(result).toBeNull();
    });

    it('returns null when upload returns errcode', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ errcode: 400001, errmsg: 'invalid media' }),
      });

      const result = await (channel as any).uploadMedia('/tmp/bad.png', 'image');
  ...
  describe('sendImage', () => {
    it('uploads and sends image to group chat', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();

      // Prime webhook cache (group)
      const msg = createRobotMessage({
        sessionWebhook: 'https://example.com/webhook',
        sessionWebhookExpiredTime: Date.now() - 1000, // expired so it uses OpenAPI
        conversationType: '2',
      });
      registeredCallback!(createDownstream(msg));
      mockFetch.mockClear();
      ...
      // 1. Access token (for upload)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ accessToken: 'tok_si', expireIn: 7200 }),
      });
      // 2. Upload
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ media_id: '@media_456', errcode: 0 }),
      });
      // 3. Send via group OpenAPI
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({}),
        text: () => Promise.resolve('ok'),
      ...
      await channel.sendImage('conv123@dingtalk', '/tmp/photo.png');

      // Verify upload call
      expect(mockFetch.mock.calls[1][0]).toContain('oapi.dingtalk.com/media/upload');
      expect(mockFetch.mock.calls[1][0]).toContain('type=image');

      // Verify send call with sampleImageMsg
      const sendBody = JSON.parse(mockFetch.mock.calls[2][1].body);
      expect(sendBody.msgKey).toBe('sampleImageMsg');
      expect(JSON.parse(sendBody.msgParam)).toEqual({ photoURL: '@media_456' });
      expect(sendBody.openConversationId).toBe('conv123');
    });

    it('uploads and sends image to DM chat', async () => {
      const opts = createTestOpts({
        registeredGroups: vi.fn(() => ({
          'dm_img@dingtalk': {
            name: 'DM Img',
      ...
      // Prime DM cache
      const msg = createRobotMessage({
        conversationId: 'dm_img',
        conversationType: '1',
        senderStaffId: 'staff_img',
        sessionWebhookExpiredTime: Date.now() - 1000,
      });
      registeredCallback!(createDownstream(msg));
      mockFetch.mockClear();
      ...
      // Token, upload, DM send
      mockFetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ accessToken: 'tok', expireIn: 7200 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ media_id: '@dm_media', errcode: 0 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}), text: () => Promise.resolve('ok') });

      await channel.sendImage('dm_img@dingtalk', '/tmp/dm-photo.png');

      // Should use DM endpoint
      expect(mockFetch.mock.calls[2][0]).toBe('https://api.dingtalk.com/v1.0/robot/oToMessages/batchSend');
      const dmBody = JSON.parse(mockFetch.mock.calls[2][1].body);
      expect(dmBody.msgKey).toBe('sampleImageMsg');
      expect(dmBody.userIds).toEqual(['staff_img']);
    });

    it('sends caption as follow-up message', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();

      // Prime webhook cache (group, valid webhook)
      const msg = createRobotMessage({
        sessionWebhook: 'https://example.com/webhook',
        sessionWebhookExpiredTime: Date.now() + 3600000,
        conversationType: '2',
      ...
      // Token (upload), upload, group send (image), webhook send (caption)
      mockFetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ accessToken: 'tok', expireIn: 7200 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ media_id: '@cap_media', errcode: 0 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}), text: () => Promise.resolve('ok') })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ errcode: 0, errmsg: 'ok' }), text: () => Promise.resolve('ok') });

      await channel.sendImage('conv123@dingtalk', '/tmp/photo.png', 'Check this out!');

      // 4th call should be the caption sent via webhook
      expect(mockFetch).toHaveBeenCalledTimes(4);
      const captionBody = JSON.parse(mockFetch.mock.calls[3][1].body);
      expect(captionBody.markdown.text).toBe('Check this out!');
    });
    ...
    it('handles upload failure gracefully', async () => {
      ...
      // Should not throw
      await expect(channel.sendImage('conv123@dingtalk', '/tmp/bad.png')).resolves.toBeUndefined();
  ...
  describe('sendFile', () => {
    it('uploads and sends file to group chat', async () => {
      const channel = new DingTalkChannel(createTestOpts());
      await channel.connect();
      ...
      // Token, upload, send
      mockFetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ accessToken: 'tok_f', expireIn: 7200 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ media_id: '@file_media_1', errcode: 0 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}), text: () => Promise.resolve('ok') });

      await channel.sendFile('conv123@dingtalk', '/tmp/report.pdf', 'report.pdf');

      // Verify upload used type=file
      expect(mockFetch.mock.calls[1][0]).toContain('type=file');

      // Verify send used sampleFile
      const sendBody = JSON.parse(mockFetch.mock.calls[2][1].body);
      expect(sendBody.msgKey).toBe('sampleFile');
      const param = JSON.parse(sendBody.msgParam);
      expect(param.mediaId).toBe('@file_media_1');
      expect(param.fileName).toBe('report.pdf');
      expect(param.fileType).toBe('pdf');
    });

    it('uploads and sends file to DM chat', async () => {
      const opts = createTestOpts({
        registeredGroups: vi.fn(() => ({
          'dm_file@dingtalk': {
            name: 'DM File',
      ...
      mockReadFileSync.mockReturnValue(Buffer.from('file'));

      // Token, upload, DM send
      mockFetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ accessToken: 'tok', expireIn: 7200 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ media_id: '@dm_file_m', errcode: 0 }) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}), text: () => Promise.resolve('ok') });

      await channel.sendFile('dm_file@dingtalk', '/tmp/doc.docx', 'doc.docx');

      expect(mockFetch.mock.calls[2][0]).toBe('https://api.dingtalk.com/v1.0/robot/oToMessages/batchSend');
      const body = JSON.parse(mockFetch.mock.calls[2][1].body);
      expect(body.msgKey).toBe('sampleFile');
      expect(body.userIds).toEqual(['staff_file']);
    });
  });
});
