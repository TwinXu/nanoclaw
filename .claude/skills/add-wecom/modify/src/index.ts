...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  if (!isMainGroup && group.requiresTrigger !== false) {
    const hasTrigger = missedMessages.some((m) =>
      m.is_bot_mentioned || TRIGGER_PATTERN.test(m.content.trim()),
    );
...
async function main(): Promise<void> {
  ...
  // WeCom (if relay URL and credentials configured)
  if (WECOM_RELAY_URL && WECOM_CORP_ID) {
  ...
  if (channels.length === 0) {
    logger.error('No channels configured. Set WhatsApp, Feishu, DingTalk, or WeCom credentials.');
    process.exit(1);
  }
